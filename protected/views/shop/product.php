		...
		<?php if(!Yii::app()->user->isGuest) echo CHtml::link('редактировать', array('cp/shop/productUpdate', 'id' =>$product->id), array('class'=>'btn-product-edit', 'target' => 'blank'));?>
			<div class="js__main-photo product-main-img">
				...
				<? foreach(['sale','new','hit'] as $mark) {
					if($product->$mark) { echo \CHtml::tag('span', ['class'=>"product__mark-{$mark}"], '', true); }
				} ?>
				</div>
				<?php if($product->mainImageBehavior->isEnabled()): ?>
					<?=CHtml::link(CHtml::image($product->getSrc()), $product->mainImageBehavior->getSrc(), ['class'=>'image-full', 'data-fancybox'=>'group']); ?>
					<?else:?>
...
<div class="buy">
    ...
    <?if($product->notexist):?>
    	<div class="shop-button btn notexist">нет в наличии</div>
    <?else:?>
